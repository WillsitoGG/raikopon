...
def main() -> int:
    ...
    if original[0x10:0x14] != b"NRO0": raise RuntimeError("Input is not an NRO")
    if sha256(original) != UPSTREAM_SHA256: raise RuntimeError("Input is not the exact official Raikopon v2.5.0 release NRO")
    if original.count(OLD) != 1: raise RuntimeError(f"Expected exactly one default user-root literal; found {original.count(OLD)}")
    pos = original.index(OLD)
    if pos != EXPECTED_OFFSET: raise RuntimeError(f"Unexpected default user-root offset: 0x{pos:X}")
    growth = len(NEW) - len(OLD)
    if growth != 2: raise RuntimeError(f"Unexpected growth: {growth}")
    padding = original[pos + len(OLD):pos + len(OLD) + growth]
    if padding != b"\x00" * growth: raise RuntimeError("Required zero padding is not present after default user-root literal")
    for marker in PRESERVE:
        if marker not in original: raise RuntimeError(f"Expected upstream marker missing: {marker!r}")
    patched = bytearray(original)
    patched[pos:pos + len(NEW)] = NEW
    output = bytes(patched)
    if len(output) != len(original): raise RuntimeError("Output size changed")
    if output[0x10:0x14] != b"NRO0": raise RuntimeError("Output lost NRO0")
    if OLD in output: raise RuntimeError("Old default user-root literal remains")
    if output.count(NEW) != 1: raise RuntimeError("New default user-root literal count is not exactly one")
    if output[:TEXT_END] != original[:TEXT_END]: raise RuntimeError("Executable/text segment changed")
    for marker in PRESERVE:
        if marker not in output: raise RuntimeError(f"Preserved upstream marker changed: {marker!r}")
    if b"sdmc:/switch/raikopon/roms/\x00" in output: raise RuntimeError("ROM-directory literal was unexpectedly patched")
    if b"sdmc:/switch/raikopon/user_dir.txt\x00" in output: raise RuntimeError("user_dir.txt indirection was unexpectedly patched")
    ...
    if unexpected: raise RuntimeError(f"Unexpected binary deltas outside default root literal: {unexpected[:16]}")
    out_sha = sha256(output)
    if out_sha != OUTPUT_SHA256: raise RuntimeError(f"Unexpected v3 output SHA-256: {out_sha}")
